...
def create_landscapes(tfrecord_dir, image_dir, shuffle, resolution=512):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            try:
                img = PIL.Image.open(image_dir + "/" + image_filenames[order[idx]])
                if (res != resolution):
                ...
                if channels == 1:
                    ...
                    img = img.transpose([2, 0, 1]) # HWC => CHW
                tfr.add_image(img)
                label = labels_dict[image_filenames[order[idx]]]
                labels.append(label)
            except:
                print("Exception in " + image_filenames[order[idx]])
                
            if (tfr.cur_images >= 4000000000):
                break
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_landscapes', 'Create dataset from a directory full of images.',
                                            'create_lanscapes datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    p.add_argument(     '--resolution',     help='Output resolution (default: 512)', type=int, default=512)
